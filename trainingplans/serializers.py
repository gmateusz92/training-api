...
from .models import WorkoutPlan, TrainingCategory, Exercise
...
class WorkoutPlanSerializer(serializers.ModelSerializer):
    ...
    def validate_name(self, value):
        if self.instance:
            if WorkoutPlan.objects.filter(name=value).exclude(id=self.instance.id).exists():
                raise serializers.ValidationError("Workout plan with this name already exists.")
        else:
    ...
    def to_representation(self, instance):
        ...
        representation['exercises'] = [exercise.name for exercise in instance.exercises.all()]
